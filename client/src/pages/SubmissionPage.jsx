  ...
  return (
    <main className="submission-page">
      ...
      <form className="submission-form" onSubmit={handleSubmit} noValidate>
        ...
        <fieldset className="submission-section" disabled={submitting || isRetry}>
          <div className="submission-section__heading">
            <legend>Discovery micro-tags</legend>
            <span>{selectedTags.length}/{MAX_TAGS} selected</span>
          </div>
          <p className="submission-help">
            Choose the tags that genuinely describe the experience. These are static options for now; AI suggestions arrive later.
          </p>
          {catalogueState === 'fallback' && (
            <p className="submission-catalogue-note">Using the built-in tag catalogue while the live catalogue is unavailable.</p>
          )}
          <div className="submission-tag-categories" aria-busy={catalogueState === 'loading'}>
            {Object.entries(tagCategories).map(([category, tags]) => (
              <section key={category} className="submission-tag-category">
                <h2>{category}</h2>
                <div className="submission-tag-list">
                  {tags.map((tag) => {
                    const selected = selectedTags.includes(tag);
                    const unavailable = !selected && selectedTags.length >= MAX_TAGS;
                    return (
                      <button
                        key={tag}
                        type="button"
                        className={`submission-tag ${selected ? 'submission-tag--selected' : ''}`}
